...
public class KThread {
	...
	public static void finish() {
		...
		if (currentThread.joinCaller != null) {
			currentThread.joinCaller.nextThread().ready();
		}
	...
	public void join() {
		...
		this.joinCaller = ThreadedKernel.scheduler.newThreadQueue(true);
		boolean intStatus = Machine.interrupt().disable();
		this.joinCaller.acquire (this);
		this.joinCaller.waitForAccess (currentThread);
		sleep();
		Machine.interrupt().restore(intStatus);
	...
	private static class PingTest implements Runnable {
		...
		public void run() {
			for (int i=0; i<6; i++) {
				System.out.println("PING thread " + which + " looped "
						+ i + " times");
				currentThread.yield();
			}
		}
	...
	private static class JoinTest implements Runnable {
		...
		public void run() {
			...
			KThread[] x = new KThread[6];
			for (int i=0; i<6; i++) {
				x[i] = new KThread(new PingTest(i+1));
			}
			for (int i=5; i>=0; i-=2) {
				x[i].fork();
				x[i-1].fork();
	...
	private ThreadQueue joinCaller;
